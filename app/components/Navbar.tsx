      ...
      <div className='flex items-center gap-1 justify-end w-full'>
        ...
        <ul className="flex items-center gap-5">
          <li className="relative dropdown-container" onClick={toggleUserDropdown}>
            ...
            >
               <li className=" hover:bg-gray-400 bg-white rounded-t-lg   text-gray-700 hover:text-white">
                <button
                  className={`block px-5 py-2  w-full text-left ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                  onClick={withdrawInit}
                  disabled={loading} // Disable button while loading
                >
                  {loading ? "Withdrawing..." : "Withdraw"}
                </button>
              </li>
              <li className="hover:bg-gray-400  bg-white rounded-b-lg text-gray-700 hover:text-white">
                <button
                  className={`block px-5 py-2 w-full text-left ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                  onClick={logOut}
                  disabled={loading} // Disable button while loading
